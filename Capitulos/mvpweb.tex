...
La aplicación se implementa con Streamlit y se estructura en un punto de entrada, un conjunto de páginas y varios módulos auxiliares en \texttt{src/}. El fichero \texttt{app.py} configura la página, muestra el resumen inicial, verifica los artefactos locales y define la navegación entre páginas. La navegación incluye cinco vistas: Inicio, Predicción de volatilidad, Diagnóstico de datos, Comparación de modelos y Ayuda y limitaciones.
...
Después, el usuario accede a la página de predicción y selecciona la fuente de datos. Si elige Binance API, la aplicación descarga las últimas velas cerradas de \texttt{BTCUSDT} con intervalo de cinco minutos. Si elige dataset de ejemplo, carga el fichero local incluido en \texttt{data/\allowbreak sample/}. Si sube un CSV, el archivo se normaliza aceptando una columna temporal reconocida y una columna \texttt{close}.
...
Al pulsar \textit{Generar predicciones}, el MVP carga los modelos mediante \texttt{ModelRegistry} y ejecuta solo aquellos que están disponibles. La salida incluye una tabla con estado, predicción en escala logarítmica, volatilidad realizada transformada, raíz de volatilidad en porcentaje y nota técnica de cada modelo. La aplicación también representa el contexto reciente y permite descargar los resultados en \texttt{btc\_\allowbreak volatility\_\allowbreak predictions.csv}.

De forma opcional, el usuario puede calcular la evaluación \textit{walk-forward} reciente. En ese caso, la aplicación crea targets futuros dentro de la ventana cargada solo para puntos donde ya son observables, ejecuta predicciones retrospectivas y calcula métricas por modelo. Los resultados pueden visualizarse en pestañas separadas y descargarse en \texttt{btc\_\allowbreak volatility\_\allowbreak walk\_\allowbreak forward\_\allowbreak predictions.csv}.
